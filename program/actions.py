...
def read_base(file):
# чтение содержимого справочника. возвращает список двух словарей (fio и phones)
    print('Чтение')
    ...
    return resultData, max(list(resultData[0].keys()))
...
#запись полной измененной базы обратно в файл
def save_base_(file,base):
    print('Запись')
    with open(file, 'w', encoding='utf-8') as text:
        for key in base[0].keys():
            text.write(str(key)+';'+base[0][key]+';'+base[1][key]+'\n')
